...
Section ideal_basic.
  ...
  Fact ideal_equiv 𝓘 𝓙 : 𝓘 ≡₁ 𝓙 → ideal 𝓘 → ideal 𝓙.
  Proof.
  ...
  Variables (𝓘 : _ ) (H𝓘 : ideal 𝓘).

  Fact ideal_eq x y : x ∼ᵣ y → 𝓘 x → 𝓘 y.
  Proof. apply H𝓘 . Qed.

  Fact ideal_op_a : ∀ x y, 𝓘 x → 𝓘 y → 𝓘 (x +ᵣ y).
  Proof. apply H𝓘. Qed.

  Fact ideal_iv_a x : 𝓘 x → 𝓘 (-ᵣ x).
  Proof.
    intros Hx.
  ...
  Hint Resolve ideal_iv_a : core.

  Fact ideal_sub_a x y : 𝓘 x → 𝓘 y → 𝓘 (x −ᵣ y).
  Proof. intros; apply H𝓘; auto. Qed.

End ideal_basic.

(* The ring parameter in ideal is declared implicit *)
Arguments ideal {_}.
...
Section ideal_idl_lc_extra.
  ...
  Fact idl_smallest P : ∀𝓘, ideal 𝓘 → P ⊆₁ 𝓘 → idl P ⊆₁ 𝓘.
  ...
  Proof. apply idl_smallest, idl__ideal. Qed.
