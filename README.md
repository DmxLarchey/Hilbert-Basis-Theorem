...
Then _we characterize Noetherianess as 
```coq 
...
```coq
Definition principal R := ∃g, Idl ⌞l⌟ = { x*ᵣg | x ∈ ∈ R }.
```
...
the two properties are linked in some way, ee below.

In addition to the HBT above, with these definitions, we can show that:
- fields are principal and Noetherian rings;
- the ring of integers `Z` is a principal ring and Noetherian ring;
- the quotient of a principal ring is principal;
- the quotient of a Noetherian ring is Noetherian.

## Induction principles derived from Noetherianess and obtaining finite bases
